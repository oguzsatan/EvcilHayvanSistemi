...
void loop() {
  ...
  current_time = millis();
    if (current_time - last_read_time > 10000) // 10 saniyede bir yeni SMS olup olmadığını kontrol ediyoruz
    {
      ...
      delay(10);
      if (received_SMS.indexOf("Konum") != -1)   
      // Eğer SMS içeriğinde "Konum" metni bulunuyorsa, 
      // uydudan aldığımız en son konumu Google Maps linki olarak, girilen telefon numarasına SMS gönderiyoruz
      {
        node.sendSMS("5305115298", SMS_text[0]);    // '123456789' yerine telefon numaramızı giriyoruz
        received_SMS = "";
      }
    }
